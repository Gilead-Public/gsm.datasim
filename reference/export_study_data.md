...
- overwrite:
  ...
  If `TRUE`, existing Parquet files are silently overwritten. If `FALSE`
  (default), an error is raised when the target study folder already
  exists.

- compression:

  Compression codec passed to
...
- save_rds:
  ...
  etc.) that cannot be expressed as flat Parquet files. Defaults to
  `FALSE`.
...
be preserved alongside the Parquet files by setting `save_rds = TRUE`,
which writes a companion `analytics_full.rds` per snapshot.
