...
test_that("infer_column_type uses explicit spec type when available (#89, #95)", {  expect_equal(infer_column_type("foo", list(type = "date")), "date")
...
test_that("infer_column_type falls back to name patterns (#89, #95)", {  expect_equal(infer_column_type("enroll_dt"), "date")
...
test_that("infer_column_type defaults to character (#89, #95)", {  expect_equal(infer_column_type("sitename"), "character")
...
test_that("generate_column_by_type produces correct types and lengths (#89, #95)", {  set.seed(42)
  ctx <- list(
    data       = list(),
...
test_that("generate_column_by_type respects explicit spec type over name pattern (#89, #95)", {  set.seed(42)
...
test_that("generate_column_by_type samples FK from parent domain (#89, #95)", {  set.seed(42)
...
test_that("generate_unknown_domain creates data.frame with correct shape (#89, #95)", {  set.seed(42)
  spec <- list(
...
test_that("generate_unknown_domain applies source_col renames (#89, #95)", {  set.seed(42)
  spec <- list(
...
test_that("generate_unknown_domain returns empty data.frame for n=0 (#89, #95)", {  spec <- list(col1 = list(type = "character"))
  result <- generate_unknown_domain("Raw_X", spec, 0, list())
  expect_s3_class(result, "data.frame")
  expect_equal(nrow(result), 0)
})

test_that("generate_unknown_domain uses FK from parent data (#89, #95)", {  set.seed(42)
...
test_that("generate_data_from_workflows rejects empty input (#89, #95)", {  expect_error(
    generate_data_from_workflows(list()),
    "non-empty named list"
  )
})

test_that("generate_data_from_workflows handles pure-unknown workflows (#89, #95)", {  set.seed(42)
...
test_that("generate_data_from_workflows respects domain_counts (#89, #95)", {  set.seed(42)
...
test_that("generate_data_from_workflows respects desired_domains filter (#89, #95)", {  set.seed(42)
...
test_that(".resolve_domain_counts uses heuristics for known domains (#89, #95)", {  counts <- .resolve_domain_counts(
...
test_that(".resolve_domain_counts uses participant count for unknown domains (#89, #95)", {  counts <- .resolve_domain_counts(
...
test_that(".resolve_domain_counts respects user overrides (#89, #95)", {  counts <- .resolve_domain_counts(
...
test_that("generate_data_from_workflows produces multiple snapshots (#89, #95)", {  set.seed(42)
...
test_that("longitudinal snapshots have cumulative rows for unknown domains (#89, #95)", {  set.seed(123)
...
test_that("snapshot names are end-date strings (#89, #95)", {  set.seed(42)
...
test_that("single snapshot returns flat list (backward compatible) (#89, #95)", {  set.seed(42)
...
test_that("generate_unknown_domain uses cumulative delta with previous_data (#89, #95)", {  set.seed(42)
  spec <- list(
    col_a = list(type = "character"),
...
test_that("generate_unknown_domain returns previous_data when delta <= 0 (#89, #95)", {  set.seed(42)
...
test_that(".apply_column_overrides returns df unchanged when overrides is NULL (#89, #95)", {  df <- data.frame(a = 1:5, b = letters[1:5], stringsAsFactors = FALSE)
  result <- .apply_column_overrides(df, "Raw_X", NULL)
  expect_equal(result, df)
})

test_that(".apply_column_overrides returns df unchanged when domain not in overrides (#89, #95)", {  df <- data.frame(a = 1:5, stringsAsFactors = FALSE)
  overrides <- list(Raw_OTHER = list(a = function(n) rep(99L, n)))
  result <- .apply_column_overrides(df, "Raw_X", overrides)
  expect_equal(result, df)
})

test_that(".apply_column_overrides adds new column via function(n) (#89, #95)", {  df <- data.frame(a = 1:10, stringsAsFactors = FALSE)
...
test_that(".apply_column_overrides replaces existing column via function(n) (#89, #95)", {  df <- data.frame(val = 1:5, stringsAsFactors = FALSE)
  overrides <- list(Raw_X = list(val = function(n) rep(0L, n)))
  result <- .apply_column_overrides(df, "Raw_X", overrides)
  expect_equal(result$val, rep(0L, 5))
})

test_that(".apply_column_overrides passes df to function(n, df) (#89, #95)", {  df <- data.frame(base_val = c(1, 2, 3, 4, 5), stringsAsFactors = FALSE)
...
test_that(".apply_column_overrides samples vector with replacement (#89, #95)", {  set.seed(42)
...
test_that(".apply_column_overrides broadcasts scalar to all rows (#89, #95)", {  df <- data.frame(x = 1:8, stringsAsFactors = FALSE)
  overrides <- list(Raw_X = list(category = "CHEMISTRY"))
  result <- .apply_column_overrides(df, "Raw_X", overrides)
  expect_equal(result$category, rep("CHEMISTRY", 8))
})

test_that(".apply_column_overrides handles multiple columns in one call (#89, #95)", {  df <- data.frame(a = 1:5, stringsAsFactors = FALSE)
  overrides <- list(
    Raw_X = list(
...
test_that("column_overrides function(n) adds new column to generated domain (#89, #95)", {  set.seed(42)
  result <- generate_data_from_workflows(
...
test_that("column_overrides function(n, df) can derive from existing columns (#89, #95)", {  set.seed(42)
  result <- generate_data_from_workflows(
    lWorkflows = make_override_workflows(),
    n_participants = 15,
    column_overrides = list(
...
test_that("column_overrides vector is sampled into generated domain (#89, #95)", {  set.seed(42)
  result <- generate_data_from_workflows(
    lWorkflows = make_override_workflows(),
    n_participants = 30,
    column_overrides = list(
...
test_that("column_overrides scalar is broadcast to all rows (#89, #95)", {  set.seed(42)
  result <- generate_data_from_workflows(
    lWorkflows = make_override_workflows(),
    n_participants = 10,
    column_overrides = list(
...
test_that("column_overrides replaces an existing column (#89, #95)", {  set.seed(42)
  result <- generate_data_from_workflows(
    lWorkflows = make_override_workflows(),
    n_participants = 10,
    column_overrides = list(
...
test_that("column_overrides applies to multiple domains independently (#89, #95)", {  set.seed(42)
  wf <- list(
    wf1 = list(
      meta = list(),
      spec = list(
...
test_that("column_overrides NULL leaves output unchanged (backward compat) (#89, #95)", {  set.seed(42)
