...
- New test data bundle tooling (gsm.roadmap#118):
  [`build_testdata_bundle()`](https://gilead-public.github.io/gsm.datasim/dev/reference/build_testdata_bundle.md)
  reproduces the shared gsm test data that used to ship inside
  `gsm.core` (`lSource` and the `reporting*` / `analytics*` objects)
  from the checked-in `inst/testdata/bundle-config.yaml` (read with
  [`read_testdata_config()`](https://gilead-public.github.io/gsm.datasim/dev/reference/read_testdata_config.md)),
  writes a four-layer bundle (raw / mapped / analytics / reporting,
  three snapshots) and a `manifest.json` with provenance, package
  versions and per-file SHA-256 checksums. The two edits the old
  maintainer script applied silently (`Raw_SITE$site_status`,
  `SnapshotDate` stamping) are explicit, configurable steps
  ([\#151](https://github.com/Gilead-Public/gsm.datasim/issues/151)).
- New
  [`explore_testdata()`](https://gilead-public.github.io/gsm.datasim/dev/reference/explore_testdata.md)
  htmlwidget and *Test Data Explorer* example page: bundle header,
  snapshot / layer / table tree, sortable and searchable table viewer
  with per-column summaries, and overview charts. Backed by
  ...
  front-end; charts use `gsm.viz`
  ([\#155](https://github.com/Gilead-Public/gsm.datasim/issues/155)).
- [`execute_analytics_pipeline()`](https://gilead-public.github.io/gsm.datasim/dev/reference/execute_analytics_pipeline.md)
  ...
  accepts a vector of analytics packages, e.g.
  `analytics_package = c("gsm.kri", "gsm.qtl")`, and concatenates their
  metric workflows.
